...
if (!fs.existsSync(binaryPath)) {
  ...
  try {
    ...
    console.log(`[check-binary] Detected binary headers: ${isWindows ? 'Windows PE' : isLinux ? 'Linux ELF' : isMac ? 'macOS Mach-O' : 'Unknown'}`);
  ...
  if (!needsRebuild) {
    try {
      ...
      console.warn('[check-binary] better-sqlite3 binary failed to load in Node.js (likely ABI mismatch):', err.message);
      needsRebuild = true;
    }
  }
}
